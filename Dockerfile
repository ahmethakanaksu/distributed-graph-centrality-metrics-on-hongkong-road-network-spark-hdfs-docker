...
RUN curl https://archive.apache.org/dist/spark/spark-${SPARK_VERSION}/spark-${SPARK_VERSION}-bin-hadoop3.tgz -o spark-${SPARK_VERSION}-bin-hadoop3.tgz \
 && tar -xvzf spark-${SPARK_VERSION}-bin-hadoop3.tgz -C /opt/spark --strip-components=1 \
 && rm -rf spark-${SPARK_VERSION}-bin-hadoop3.tgz

# Download and install Hadoop
RUN curl https://dlcdn.apache.org/hadoop/common/hadoop-3.3.5/hadoop-3.3.5.tar.gz -o hadoop-3.3.5-bin.tar.gz && tar -xvzf hadoop-3.3.5-bin.tar.gz -C /opt/hadoop --strip-components=1\
...
COPY ./Hongkong.road-d ./Hongkong.road-d
COPY ./Hongkong.road-t ./Hongkong.road-t

RUN chmod -R 777 ./Hongkong.co ./Hongkong.road-d ./Hongkong.road-t
